...
const chatHandler = (io, socket) => {
  ...
  socket.on('send_message', async (data) => {
    try {
      ...
      const conversation = await chatModel.getConversationById(conversationId, socket.userId);
      if (!conversation) {
      ...
      const messageData = {
        ...
        message: (messageType === 'image' || messageType === 'file') 
      ...
      console.log(`Message sent from ${socket.userId} to ${receiverId} (type: ${messageType})`);
    } catch (error) {
  ...
  socket.on('mark_as_read', async (data) => {
    try {
      ...
      const updatedMessages = await chatModel.markMessagesAsRead(messageIds, socket.userId);

      // Get unique sender IDs
      const senderIds = [...new Set(updatedMessages.map(msg => msg.sender_id))];
